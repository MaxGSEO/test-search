...
TEXT_RAZOR_KEY = "f0852f0515c92c0686d129f79ff9c5584b87c1b67d503073a4c93ec9"

client = RestClient("virtuoso.irfan@gmail.com", "e7364dafacb0b57a")
hide_st_style = """
            <style>
...
def get_df_entities_razor(text_razor_key, text_input, lang, is_url, scrape_all):
    """ Get data using TextRazor API.
    Args:
        text_razor_key (str): TextRazor API key.
        text_input (str): Text to analyze.
...
# get live search results using dataforseo api
def get_live_search_results(query, lan, loc):
    post_data = dict()
    ...
    if response["status_code"] == 20000:
        ...
        for index, item in enumerate(items):
            if "url" in item and "description" in item and "title" in item and "rank_absolute" in item:
                if item["url"]:
                    urls.append(item["url"])
                    descriptions.append(item["description"])
                    titles.append(item["title"])
                    ranking_positions.append(item["rank_absolute"])

        return pd.DataFrame({"urls": urls, "titles": titles, "ranking position": ranking_positions, "descriptions": descriptions})
    else:
        print("error. Code: %d Message: %s" % (response["status_code"], response["status_message"]))


def get_entities(selected_rows, loc, lang):
    results = []
    for row in selected_rows:
        url = row["urls"]
        entities, response = get_df_entities_razor(TEXT_RAZOR_KEY, url, lang, True, False)
